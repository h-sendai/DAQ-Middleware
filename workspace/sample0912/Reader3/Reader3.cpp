...
static const char* reader3_spec[] =
{
    "implementation_id", "Reader3",
    "type_name",         "Reader3",
    "description",       "Reader3 component",
    "version",           "1.0",
...
Reader3::Reader3(RTC::Manager* manager)
    : DAQMW::DaqComponentBase(manager),
      m_OutPort("reader3_out", m_out_data),
      m_sock(0),
      m_recv_byte_size(0),
      ...
      m_debug(false)
{
    ...
    // Set OutPort buffers
    registerOutPort("reader3_out", m_OutPort);

    init_command_port();
    init_state_table();
    set_comp_name("READER3");
}
...
RTC::ReturnCode_t Reader3::onInitialize()
{
    if (m_debug) {
        std::cerr << "Reader3::onInitialize()" << std::endl;
    }

    return RTC::RTC_OK;
}

RTC::ReturnCode_t Reader3::onExecute(RTC::UniqueId ec_id)
{
    daq_do();
...
int Reader3::daq_dummy()
{
    return 0;
}

int Reader3::daq_configure()
{
    std::cerr << "*** Reader3::configure" << std::endl;
...
int Reader3::parse_params(::NVList* list)
{
...
int Reader3::daq_unconfigure()
{
    std::cerr << "*** Reader3::unconfigure" << std::endl;

    return 0;
}

int Reader3::daq_start()
{
    std::cerr << "*** Reader3::start" << std::endl;
...
int Reader3::daq_stop()
{
    std::cerr << "*** Reader3::stop" << std::endl;
...
int Reader3::daq_pause()
{
    std::cerr << "*** Reader3::pause" << std::endl;

    return 0;
}

int Reader3::daq_resume()
{
    std::cerr << "*** Reader3::resume" << std::endl;

    return 0;
}

int Reader3::get_data_length(unsigned char *buf, int buflen)
{
    unsigned int *length;
    unsigned int rv;
...
int Reader3::read_data_from_detectors()
{
    int status;
    int data_length = 0;
...
int Reader3::set_data(unsigned int data_byte_size)
{
...
extern "C"
{
    ...
    {
        RTC::Properties profile(reader3_spec);
