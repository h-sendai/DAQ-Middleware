...
static const char* mydaqlogger_spec[] = {
    "implementation_id", "DAQLogger",
    "type_name",         "DAQLogger",
...
DAQLogger::DAQLogger(RTC::Manager* manager)
    : DAQMW::DaqComponentBase(manager),
      m_InPort("daqlogger_in", m_in_data),
      ...
      m_debug(false)
{
    ...
    registerInPort("daqlogger_in", m_InPort);

    init_command_port();
    init_state_table();
    set_comp_name("DAQLogger");
}
...
RTC::ReturnCode_t DAQLogger::onInitialize()
{
    if (m_debug) {
        std::cerr << "DAQLogger::onInitialize()" << std::endl;
    }

    return RTC::RTC_OK;
}

RTC::ReturnCode_t DAQLogger::onExecute(RTC::UniqueId ec_id)
{
    daq_do();
...
int DAQLogger::daq_dummy()
{
    return 0;
}

int DAQLogger::daq_configure()
{
    std::cerr << "*** DAQLogger::configure" << std::endl;
    int ret = 0;
    m_isDataLogging = false;
...
int DAQLogger::parse_params(::NVList* list)
{
    int ret = 0;
    ...
    for (int i = 0; i < length; i += 2) {
        ...
        if (sname == "isLogging") {
            ...
            if (svalue == "yes") {
                ...
                isExistParamLogging = true;
                std::cerr << "DAQLogger: Save to file: true\n";
            }
            else if (svalue == "no") {
                m_isDataLogging = false;
                isExistParamLogging = true;
                std::cerr << "DAQLogger: Save to file: false\n";
            }
        }
...
int DAQLogger::daq_unconfigure()
{
    std::cerr << "*** DAQLogger::unconfigure" << std::endl;
    if (m_isDataLogging) {
        delete fileUtils;
        if (m_debug) {
...
int DAQLogger::daq_start()
{
    std::cerr << "*** DAQLogger::start" << std::endl;
    ...
    if (m_isDataLogging) {
        ...
        if (ret < 0) {
            std::cerr << "### ERROR: DAQLogger: open file failed\n";
            fatal_error_report(CANNOT_OPEN_FILE);
        }
        else {
            std::cerr << "*** DAQLogger: file open succeed\n";
            m_filesOpened = true;
        }
    }
    return 0;
...
int DAQLogger::daq_stop()
{
    std::cerr << "*** DAQLogger::stop" << std::endl;

    if (m_isDataLogging && m_filesOpened) {
        if (m_debug) {
            std::cerr << "DAQLogger::stop: close files \n";
        }
...
int DAQLogger::daq_pause()
{
    std::cerr << "*** DAQLogger::pause" << std::endl;
    return 0;
}

int DAQLogger::daq_resume()
{
    std::cerr << "*** DAQLogger::resume" << std::endl;
    return 0;
}

int DAQLogger::reset_InPort()
{
    bool ret = true;
    while (ret) {
        ret = m_InPort.read();
    ...
    if (m_debug) {
        std::cerr << "*** DAQLogger::InPort flushed\n";
    }
    return 0;
}

void DAQLogger::toLower(std::basic_string<char>& s)
{
...
int DAQLogger::daq_run()
{
    ...
    if (m_isDataLogging) {
        ...
        if (ret < 0) {
            std::cerr << "### DAQLogger: ERROR occured at data saving\n";
    ...
    if (m_debug) {
        ...
        if (seq_num % m_update_rate == 0) {
            std::cerr << "DAQLogger: loop = " << seq_num << std::endl;
            std::cerr << "\033[A\r";
        }
    }
...
extern "C"
{
    void DAQLoggerInit(RTC::Manager* manager)
    {
        RTC::Properties profile(mydaqlogger_spec);
        manager->registerFactory(profile,
                                 RTC::Create<DAQLogger>,
                                 RTC::Delete<DAQLogger>);
    }
};
