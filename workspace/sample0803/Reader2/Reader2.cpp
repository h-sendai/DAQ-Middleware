...
static const char* reader2_spec[] =
{
    "implementation_id", "Reader2",
    "type_name",         "Reader2",
    "description",       "Reader2 component",
    "version",           "1.0",
...
Reader2::Reader2(RTC::Manager* manager)
    : DAQMW::DaqComponentBase(manager),
      m_OutPort("reader2_out", m_out_data),
      m_sock(0),
      m_recv_byte_size(0),
      ...
      m_debug(false)
{
    ...
    // Set OutPort buffers
    registerOutPort("reader2_out", m_OutPort);

    init_command_port();
    init_state_table();
    set_comp_name("READER2");
}
...
RTC::ReturnCode_t Reader2::onInitialize()
{
    if (m_debug) {
        std::cerr << "Reader2::onInitialize()" << std::endl;
    }

    return RTC::RTC_OK;
}

RTC::ReturnCode_t Reader2::onExecute(RTC::UniqueId ec_id)
{
    daq_do();
...
int Reader2::daq_dummy()
{
    return 0;
}

int Reader2::daq_configure()
{
    std::cerr << "*** Reader2::configure" << std::endl;
...
int Reader2::parse_params(::NVList* list)
{
...
int Reader2::daq_unconfigure()
{
    std::cerr << "*** Reader2::unconfigure" << std::endl;

    return 0;
}

int Reader2::daq_start()
{
    std::cerr << "*** Reader2::start" << std::endl;
...
int Reader2::daq_stop()
{
    std::cerr << "*** Reader2::stop" << std::endl;
...
int Reader2::daq_pause()
{
    std::cerr << "*** Reader2::pause" << std::endl;

    return 0;
}

int Reader2::daq_resume()
{
    std::cerr << "*** Reader2::resume" << std::endl;

    return 0;
}

int Reader2::read_data_from_detectors()
{
...
int Reader2::set_data(unsigned int data_byte_size)
{
...
extern "C"
{
    void Reader2Init(RTC::Manager* manager)
    {
        RTC::Properties profile(reader2_spec);
