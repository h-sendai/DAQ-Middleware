    ...
    if (FD_ISSET(0, &m_rset)) {
        ...
        case CONFIGURED:
            ...
            case CMD_UNCONFIGURE:
                ...
                break;
            case CMD_FIX:
				comp_restart_procedure(status_ptr);
				m_state = RUNNING;
                break;
            default:
                cerr << " Bad Command:" << command << endl;
        ...
        case ERROR:
			...
			case CMD_STOP:
				...
				break;
            case CMD_FIX:
                comp_stop_procedure(status_ptr);
				m_state = CONFIGURED;
				break;
			default:
				cerr << " Bad Command:" << command << endl;
        ...
        int count = 0;
        int d_err_compname_len;

        for (int i = (m_comp_num - 1); i >= 0; i--) {
			if ((d_err_compname_len = d_err_compname[i].length()) != 0) {
				++count;
...
int DaqOperator::comp_stop_procedure(Status_var *status_ptr)
{
    m_com_completed = false;
    
    try {
        for (int i = 0; i < m_comp_num; i++) {
            if ((status_ptr+i)->err_occur == true) {
...
int DaqOperator::comp_restart_procedure(Status_var *status_ptr)
{
    m_com_completed = false;
